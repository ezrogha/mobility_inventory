...
header('Content-Type: application/json');
    ...
    $result = mysqli_query($connect, "SELECT causeofdisability, count(*) FROM $distribution GROUP BY causeofdisability");
    
    $f = array("label" => " " , "y" => "0");
    
    while($row = mysqli_fetch_array($result))
    {        
        $point = array("label" => $row['causeofdisability'] , "y" => $row['count(*)']);
